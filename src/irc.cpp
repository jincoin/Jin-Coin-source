...
// Copyright (c) 2011-2012 金 Jīn Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #金 JīnTEST0\r");
            Send(hSocket, "WHO #金 JīnTEST0\r");
        } else {
            // randomly join #金 Jīn00-#金 Jīn99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #金 Jīn%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #金 Jīn%02d\r", channel_number).c_str());
        }
