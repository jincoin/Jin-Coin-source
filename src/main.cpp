...
uint256 hashGenesisBlock("0x000005195817cd43b068ee6dcd091109e937b4b5c322c6a2b23a93912e19bb76");
...
const string strMessageMagic = "Jin Signed Message:\n";
...
{
    // 金 Jīn: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 192.96 * COIN;

	// Subsidy halving
    nSubsidy >>= (nHeight / 25810);
...
static const int64 nTargetSpacing = 79;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 79;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * (79.0/60.0) * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * (79.0/60.0) * 2.8;
...
{
    // 金 Jīn: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1471801377;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1923087;
...
unsigned char pchMessageStart[4] = { 0xd7, 0xc4, 0xef, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JinMiner(CWallet *pwallet)
{
    printf("JinMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("金 Jīn-miner");
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JinMiner, pwallet));
}
